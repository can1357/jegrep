...
use crate::jev::{self, Client, Question, Response};
use crate::questions::{FileErr, FilePrep, ReadText, prepare_file, read_text};
...
pub enum Job {
    ...
    /// Just read a file's first `max_bytes` (binary-checked, UTF-8 lossy). No request.
    Read {
        id: u64,
        path: PathBuf,
        max_bytes: usize,
...
pub enum Outcome {
    Ask {
        id: u64,
        result: Result<Response, jev::Error>,
        elapsed: Duration,
    },
    File {
        id: u64,
        result: Result<(FilePrep, Response), FileErr>,
        elapsed: Duration,
    },
    Read {
        id: u64,
        result: Result<ReadText, FileErr>,
        elapsed: Duration,
    },
}
        ...
        let handles = (0..workers.max(1))
            .map(|_| {
                ...
                thread::spawn(move || {
                    loop {
                        let job = {
                            let guard = jrx.lock().unwrap_or_else(|p| p.into_inner());
                            guard.recv()
                        };
