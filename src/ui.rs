...
use crate::grouped::{self, Leaf, split_rel};
use crate::tree::{HeatRange, Kind, State, Tree};
use std::{
    cell::RefCell,
    io::{IsTerminal, Write},
    sync::{Arc, Mutex, Weak},
...
/// Progress presentation selected by the CLI; live falls back on basic terminals.
...
/// Print a durable diagnostic without letting worker output corrupt live progress.
pub fn diagnostic(message: &str) {
    let active = ACTIVE.lock().unwrap_or_else(|e| e.into_inner()).upgrade();
...
impl Ui {
    ...
    /// Whether strategy-specific activity is currently visible in the live panel.
    ...
    /// A thread-safe observer for parallel local scans, independent of this UI's `RefCells`.
    ...
    pub fn name_scored(&self, path: &str, score: Option<f64>) {
        self.update(Event::Name {
            path,
            state: score.map_or(RangeState::Failed, RangeState::Scored),
        });
    }
    ...
    pub fn range_scored(&self, rel: &str, start: usize, end: usize, score: Option<f64>) {
        self.range(
            ...
            score.map_or(RangeState::Failed, RangeState::Scored),
        );
    }
    ...
    /// Stop animation and erase its owned lines before stdout receives the report.
    pub fn finish(&self) {
        let live = self.live.borrow_mut().take();
        if let Some(live) = live.as_ref() {
            live.stop();
            *ACTIVE.lock().unwrap_or_else(|e| e.into_inner()) = Weak::new();
        }
        drop(live);
    }
    ...
    pub fn banner(&self, query: &str, root: &str, model: &str, p: usize, n: usize, cap: usize) {
        ...
        if self.progress == Progress::Live && !self.is_live() {
            ...
            *ACTIVE.lock().unwrap_or_else(|e| e.into_inner()) = Arc::downgrade(&live);
            *self.live.borrow_mut() = Some(live);
        }
    }
    pub fn round(&self, round: u8, tau: f64) {
        if self.update(Event::Round {
            number: round,
            threshold: tau,
        }) {
            return;
    ...
    pub fn round_end(&self, round: u8, tau: f64, hits: usize, next: Option<f64>) {
        ...
        self.line(&self.dim(&format!(
                "── round {round} done: {hits} hit(s) at τ={tau:.2} {tail}"
            )));
    }
    pub fn batch(&self, id: u64, n: usize, in_flight: usize) {
        ...
        self.line(&format!(
            ...
            self.blue("⟳"),
            self.dim(&format!(
                "batch #{id}: {n} entries → jev   ({in_flight} in flight)"
            ))
        ));
    }
    pub fn batch_inline(&self, id: u64, entries: usize, files: usize, kb: f64, in_flight: usize) {
        self.line(&format!(
            "  {} {}",
            self.blue("⟳"),
            self.dim(&format!("batch #{id}: {entries} entries + {files} files ({kb:.0} KB) → jev   ({in_flight} in flight)"))
        ));
    }
    ...
    pub fn sniff_batch(&self, id: u64, n: usize, in_flight: usize) {
        self.line(&format!(
            ...
            self.blue("⟳"),
            self.dim(&format!(
                "sniff #{id}: {n} file heads → jev   ({in_flight} in flight)"
            ))
        ));
    }
    ...
    fn hit_body(&self, heat: &[HeatRange], lines_seen: Option<(usize, bool)>) -> Vec<String> {
        ...
        sorted.truncate(3);
        // One normalized heat glyph is always a full block, which adds no information.
        if heat.len() > 1 {
    ...
    /// Rank the final display independently of the search's best-first work order.
    pub fn print_hits(&self, out: &mut impl Write, tree: &Tree, hits: &[usize]) {
        let items: Vec<_> = hits
            ...
            .map(|&i| {
                ...
                (
                    score,
                    self.hit_leaf(&n.rel, score, n.name_score, &n.heat, n.lines_seen),
                )
            })
            .collect();
        ...
        for (_, leaf) in ordered {
            ...
            if current != Some(dir) {
                ...
                let _ = writeln!(
                    out,
                    " {}",
                    self.bold(&self.blue(if dir.is_empty() { "./" } else { dir }))
                );
                current = Some(dir);
            }
    ...
    pub fn print_tree(&self, out: &mut impl Write, tree: &Tree) {
        ...
        let mut dir_ann: std::collections::HashMap<String, String> =
...
mod tests {
    ...
    fn whole_file_hits_are_one_row_without_redundant_localization() {
        ...
        let leaf = ui.hit_leaf(
            "src/modes/hashline/parser.rs",
            0.97,
            Some(0.71),
            &heat,
            Some((854, false)),
