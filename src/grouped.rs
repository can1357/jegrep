//! Port of pi's grouped file output
...
//! - single-child directory chains fold into one header (`pkg/ai/`, then
//!   `src/util/`)
...
//!
//! Leaves are usually files; a leaf whose path ends in `/` is a collapsed
//! folder shown in place (so judged-but-unexpanded folders can be listed like
//! files).
...
impl Node {
	fn child(&mut self, seg: &str) -> &mut Self {
		let pos = if let Some(p) = self.subdirs.iter().position(|(n, _)| n == seg) {
			p
		} else {
...
pub struct Leaf {
	...
	pub header: String,
	/// Body lines under the header (already formatted; indented by the
	/// renderer).
	pub body:   Vec<String>,
}

pub enum Event<'a> {
	/// A (possibly folded) directory header. `full` is the root-relative path
	/// with trailing `/`.
	Dir {
		depth: usize,
		chain: String,
...
fn walk<'a>(node: &Node, leaves: &'a [Leaf], depth: usize, prefix: &str, out: &mut Vec<Event<'a>>) {
	for (name, i) in &node.leaves {
		out.push(Event::Leaf { depth, name: name.clone(), leaf: &leaves[*i] });
	}
	for (name, sub) in &node.subdirs {
		...
		out.push(Event::Dir { depth, chain, full: full.clone() });
...
/// Split a root-relative path into (parent dir with trailing `/` or "", last
/// segment). A trailing `/` stays on the segment so folder leaves read as
/// `name/`.
...
/// (`formatGroupedFiles(...).model`): `#` per depth, `# dir/` headers, leaf
/// lines produced by `leaf_line(leaf, name)`, a blank line before every
/// directory header and every root-level leaf after the first line.
