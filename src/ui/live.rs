//! A small stderr panel. Search events update the model; a timer paints it while
//! network calls block. No raw mode, alternate screen, or persistent cursor modes.
...
impl Live {
    pub fn new(color: bool) -> Self {
        ...
        let worker = thread::spawn(move || {
            let mut state = painter.state.lock().unwrap_or_else(|e| e.into_inner());
            while !state.stopped {
