//! Shared-path exploration tracks. Files keep their places as their ranges
//! settle.
...
const fn settled(state: RangeState) -> bool {
	matches!(state, RangeState::Scored(_) | RangeState::Pruned | RangeState::Failed)
}
...
impl Model {
	...
	fn folder(&mut self, path: &str) {
		...
		for (i, _) in path.match_indices('/') {
			...
			if !self.folders.contains_key(prefix) {
				...
				self.rollup(prefix, Counts::default(), Counts { folders: 1, ..Counts::default() });
	...
	pub(super) fn update(&mut self, event: Event<'_>) {
		match event {
			Event::Workspace { name, folders, root_files } => {
			...
			Event::Scanned(path) => {
				...
				let count = Counts { scanned: 1, ..Counts::default() };
			...
			Event::Entry { activity: Activity::Folder | Activity::Collapsed, path, .. } => {
				// Some strategies use synthetic labels such as "(pool)" for
				// batches.
				...
				if !folder.listed {
					folder.listed = true;
					self.rollup(&path, Counts::default(), Counts { listed: 1, ..Counts::default() });
				}
			},
			Event::Entry { activity: Activity::Note, detail, .. } => {
				self.status = clean(detail);
			},
			Event::Entry { activity, path, score, detail } => {
			...
			Event::Range { path, start, end, state } => {
	...
	fn phase_line(&self, spin: char, color: bool) -> String {
		...
		let names = self
			.names
			.map_or_else(|| "names".into(), |(done, total, _)| format!("names {done}/{total}"));
		let names = format!(
			"{} {names}",
		...
		format!(
			"{}  {}  {}  {}  {}",
			paint(if self.phase == Phase::Scan { "36" } else { "2" }, &scan, color),
			paint("2", "›", color),
			paint(
				if self.phase == Phase::Names {
					"36"
	...
	pub(super) fn lines(
		...
		let mut lines = vec![
			format!(
				...
				paint("2", &format!("{:.1}s", self.started.elapsed().as_secs_f64()), color)
			),
		...
		let mut budget = height.saturating_sub(4 + visible * 2 + usize::from(groups.len() > visible));
		...
		if groups.len() > visible {
			lines.push(format!(
				" {}",
				paint("2", &format!("+{} other folders being explored", groups.len() - visible), color)
			));
		}
		if groups.is_empty() {
			...
			for path in self.outline.iter().take(shown) {
				...
				let trail: String = (0..cells)
					.map(|i| {
						...
						paint(if visited { "34" } else { "2" }, if visited { "•" } else { "·" }, color)
					})
					.collect();
				lines.push(format!(
					"   {} {}{}  {}  {}",
...
fn folder_title(path: &str, color: bool) -> String {
	...
	format!("{}{}", paint("2;34", prefix, color), paint("1;34", &path[prefix.len()..], color))
}
...
mod tests {
	...
	fn fixture() -> Model {
		...
		m.update(Event::Names { done: 64, total: 128, active: 2 });
		...
		for (start, state) in [
			...
			m.update(Event::Range { path: "src/config/解決器.rs", start, end: start + 19, state });
		}
		m
	}
	...
	fn content_work_animates_after_filename_scoring_and_settles_after_failure() {
		...
		m.update(Event::Name { path, state: RangeState::Scored(0.9) });
		reading(&mut m, path);
		let first = m.file_track(&m.entries[path], 16, false, 0);
		let next = m.file_track(&m.entries[path], 16, false, 1);
		assert_ne!(first, next);
		m.update(Event::Range { path, start: 1, end: 80, state: RangeState::Failed });
		m.update(Event::Entry { activity: Activity::Skipped, path, score: None, detail: "failed" });
		let lines = m.lines(96, 24, false, 0).join("\n");
	...
	fn discoveries_stay_inline_without_a_border_or_separate_result_list() {
		...
		for (path, score) in [("src/b.rs", 0.97), ("lib/a.rs", 0.59), ("src/a.rs", 0.97)] {
			m.update(Event::Entry { activity: Activity::Hit, path, score: Some(score), detail: "" });
		}
	...
	fn repeated_events_and_reopened_files_do_not_double_count() {
		...
		for _ in 0..3 {
			m.update(Event::Name { path, state: RangeState::Scored(0.8) });
			m.update(Event::Entry { activity: Activity::Hit, path, score: Some(0.9), detail: "" });
		}
		let counts = m.folders["src/"].totals;
		assert_eq!(
			(counts.hits, counts.files, counts.checked, counts.names, counts.named),
			(1, 1, 1, 1, 1)
		);
		reading(&mut m, path);
		let counts = m.folders["src/"].totals;
		assert_eq!((counts.hits, counts.files, counts.checked, counts.ranges), (1, 1, 0, 0));
	}
	...
	fn names_scan_and_root_files_roll_up_without_sibling_prefix_collisions() {
		...
		for path in ["src/a.rs", "src-more/b.rs", "root.rs"] {
			...
			m.update(Event::Name { path, state: RangeState::Queued });
			m.update(Event::Name { path, state: RangeState::Active });
		}
		assert_eq!(m.folders[""].totals.scanned, 3);
		assert_eq!(m.counts("src/").scanned, 1);
		assert_eq!(m.counts("").names, 1);
		m.update(Event::Name { path: "src/a.rs", state: RangeState::Failed });
	...
	fn range_states_and_original_coordinates_remain_visible_without_row_rotation() {
		...
		assert!(first.contains("解決器.rs") && first.contains("L21–40"), "{first}");
