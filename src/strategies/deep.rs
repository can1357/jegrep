...
//!
//! - **Guarded fill.** While the frontier is smaller than N, eagerly list
//!   unjudged folders (shallowest first) — but never one whose child count
//!   would push the frontier past 2N. A giant cold folder (`test/` with 741
//!   entries) then costs one Noul instead of 741, and is only opened if it
//!   scores hot.
//! - **Deep expand.** A hot folder is listed *recursively* (BFS, capped at
//!   `DEEP_CAP` entries) and every file under it is judged in the same wave.
//!   Sub- folders inside the cap are opened speculatively rather than judged;
//!   folders beyond the cap stay as single judgeable entries.
//! - **Small-first reads.** A name-hot file is read at `small_first` bytes
//!   first (default 8 KB). Only a borderline result on a truncated file is
//!   upgraded to the full read. Misses (the common case) never pay for 32 KB.
//! - **Lexical speculation (opt-in).** Query tokens vs. path tokens is a free
//!   local signal. Folders whose name matches a rare query token are opened
//!   before their judgment returns (their files join the same wave); the
//!   best-matching files start a small read right away. Jev still judges every
//!   one of them on content — this only removes a round trip when the name says
//!   what the query says. Measured: saves ≤1 wave on some cases, costs extra
//!   reads on all; off.
//! - Same threshold rounds as the baseline; reopening uses cached scores. If
//!   the lowest configured bar finds nothing, one extra round runs at half of
//!   it.
//!
...
//!   `JEGREP_SMALL`=<bytes>   first-read size; 0 disables small-first (default
//! 8192)   `JEGREP_OUTLINE=1`       add a depth-≤2 folder outline of the repo
//! to batch state   `JEGREP_DEEP_CAP`=<n>    entries listed under one hot
//! folder (default 400)   `JEGREP_SPEC=1`          enable lexical speculation
//! (off by default: on the bench it                         cost +5–10 requests
//! per case for at most one saved wave)
...
/// and the head already looked faintly relevant. (A name-based rule was tried
/// and dropped: it fired on many 0.04–0.09 heads and never produced a hit.)
const UPGRADE_FLOOR: f64 = 0.10;
/// In a later (lower-τ) round, a small-only miss is re-read in full when its
/// name score was at least this — it looked promising but the head didn't show
/// it.
...
/// files (or more than `SPEC_DF_MAX` files): "client", "config", "export" are
/// not clues; "sql", "stdio", "tts" are. (12 / 2% let "export"/"html" read 21
/// files for the html case where 9 sufficed; tightened.)
...
pub struct Deep {
	...
	next_id:      u64,
	round:        u8,
	/// Bar for reading a file by name and for accepting content (with
	/// `CONTENT_FLOOR`).
	tau:          f64,
	/// Bar for opening a folder by name. Equal to `tau` except in the fallback
	/// round.
	dir_tau:      f64,
	hits:         usize,
...
impl Default for Deep {
	fn default() -> Self {
		Self {
			...
			outline:      std::env::var("JEGREP_OUTLINE").is_ok_and(|v| v == "1"),
			speculate:    std::env::var("JEGREP_SPEC").is_ok_and(|v| v == "1"),
...
impl Strategy for Deep {
	fn run(&mut self, ctx: &mut Ctx) {
		...
		while r < rounds.len() {
			...
			self.run_round(ctx);
			// Nothing at the lowest configured bar: one extra round that only
			// loosens the folder-open bar (hidden folders are the usual
			// failure), keeping the file-read bar where it was.
...
/// Crude stem: the first five characters of longer tokens (resolver ~ resolved,
/// download ~ downloader).
fn stem(t: &str) -> &str {
	if t.chars().count() >= 6 {
...
impl Deep {
	...
	/// Guarded fill: list shallowest unjudged folders until the frontier reaches
	/// N, skipping any folder that would push it past 2N.
	...
	fn dispatch(&mut self, ctx: &mut Ctx) {
		...
		if !self.frontier.is_empty() && (self.frontier.len() >= ctx.opts.batch || idle) {
			...
			// Request latency grows superlinearly with entry count (measured:
			// 29→0.5s, 162→0.7s, 255→1.6s), so aim for the soft target N per
			// request and split evenly rather than filling to the hard cap.
		...
		while self.in_flight < ctx.opts.parallel {
			...
			let wave = match &job {
				...
				Job::File { id, .. } | Job::Read { id, .. } => self.files.get(id).map_or(0, |m| m.wave),
			};
	...
	/// Lexical speculation over a frontier about to be judged: folders whose
	/// name shares a token with the query are deep-expanded now (their files
	/// join this same wave), and the best-matching files start a small read
	/// immediately. Jev still judges every one of them; this only removes a
	/// round trip.
	fn speculate(&mut self, ctx: &mut Ctx, all: &mut Vec<usize>, wave: u32) {
		...
		dirs.sort_by_key(|&(s, i)| {
			(std::cmp::Reverse(s), ctx.tree.nodes[i].depth, ctx.tree.nodes[i].rel.len())
		});
		for (_, d) in dirs.into_iter().take(SPEC_DIRS) {
			...
			ctx.ui.note_entry(
				...
				None,
				&format!("name matches the query → opened early, {} entries", kids.len()),
			);
			self.spec_dirs += 1;
			all.extend(kids);
		}
				...
				if let Some(b) = self.batches.remove(&id) {
					match result {
						...
						Err(e) => {
							...
							ctx.ui
								.error(&format!("directory batch of {} failed: {e}", b.entries.len()));
					...
					let Stage::SmallAsk { ranges, total_lines, bytes, truncated } = m.stage else {
						return;
					};
					match result {
						Ok(resp) => {
							...
							ctx.record_content(m.node, c, heat, conf, (total_lines, truncated), bytes);
							if self.accept(c) {
			...
			Outcome::Read { id, result, .. } => {
				...
				match result {
					Ok(rt) => {
						...
						self.next_id += 1;
						self.files.insert(self.next_id, FileMeta {
							node:  m.node,
							wave:  m.wave,
						...
						self
							.ready_files
							.push_front(Job::Ask { id: self.next_id, state, questions: qs });
					},
					Err(e) => {
						ctx.ui.skip(&ctx.tree.nodes[m.node].rel, &e.to_string());
						ctx.mark_skip(m.node, e.to_string());
	...
	/// List a hot folder's whole subtree (BFS) up to `deep_cap` entries. Files
	/// and over-cap folders join the frontier; in-cap folders are opened
	/// speculatively.
	fn deep_expand(&mut self, ctx: &mut Ctx, idx: usize, wave: u32, why: &str) {
		...
		ctx.ui.exp(
			...
			p,
			added.len(),
			&format!("{why}(deep: {} folders listed)", ctx.stats.expanded - before),
		);
		self.frontier.extend(added);
		self.frontier_gen = self.frontier_gen.max(wave);
	}

	/// BFS-list a folder's subtree up to `deep_cap` entries. Returns the entries
	/// to judge: all files, plus folders that did not fit under the cap.
	...
	fn queue_file(&mut self, ctx: &mut Ctx, idx: usize, wave: u32) {
		...
		if self.small_first > 0 && n.size as usize > SMALL_DIRECT_FACTOR * self.small_first {
			self
				.files
				.insert(self.next_id, FileMeta { node: idx, wave, stage: Stage::SmallRead });
			...
			self
				.files
				.insert(self.next_id, FileMeta { node: idx, wave, stage: Stage::Full });
