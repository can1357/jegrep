...
/// evidence across the passage, so deep implementation text can outrank headers.
    ...
    fn run(&mut self, ctx: &mut Ctx) {
        ...
        for batch in cards.chunks((18000 / sketch_bytes).clamp(1, 48)) {
            ...
            for (k, &(f, p)) in batch.iter().enumerate() {
                ...
                qs.insert(key.clone(),Question::Noul { instructions:Value::String(format!("Could passage {key} implement a requested step of search? Apply criteria.")),criteria:None });
            }
