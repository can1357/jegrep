...
//!   folders get a lower floor than files and exploration runs ahead of
//!   reading.
...
//!   once; everything else waits until exploration is quiescent, then the top
//!   of the backlog is read if it clears the adaptive cut (gap / ratio rule
//!   over the whole distribution of name scores seen so far). While a clearly
//!   stronger candidate is being read (score > GATE × ours), weaker ones wait.
//! - Settling instead of a hard early stop: once a CONFIDENT hit exists
//!   (content ≥ 0.7), only folders and files whose name score is ≥ that hit's
//!   own rank score are still explored (they might hold a better answer); a
//!   weak hit counts but lets the current tier finish. Any hit caps further
//!   reads at K. When that drains, the search ends. No hits → the floors relax
//!   tier by tier and cached content scores are re-examined, like the baseline.
...
//!   many folders per request (~20× cheaper than listing + judging every
//!   child). A folder that lights up is re-scored and expands normally.
//!
//! Knobs (env): `JEGREP_K=8` `JEGREP_CAP=40` `JEGREP_D=8` `JEGREP_HI=0.7`
//! `JEGREP_GATE=0.9` `JEGREP_CONFIDENT=0.7` `JEGREP_TEST_DEMOTE=0.1`
//! `JEGREP_DIR_RATIO=0.5` `JEGREP_GIANT=2` `JEGREP_PEEK_FLOOR=0.05`
//! `JEGREP_RULE=gap|ratio|none`.
...
/// Per relaxation tier: (folder-expansion floor, file-read floor, content hit
/// bar). Expanding a folder costs ~50 tokens per child; reading a file ~10k
/// tokens, so folders get a much lower bar than files.
...
pub struct Budget {
	...
	d:           usize,
	hi:          f64,
	/// Hold a candidate whose name score is below `gate` × the best outstanding
	/// read.
	gate:        f64,
	...
	best_dir:     f64,
	/// Folders already judged from their full listing; ask id → folders in
	/// flight.
...
impl Budget {
	...
	fn enter_tier(&mut self, ctx: &mut Ctx, tier: usize) {
		...
		if tier > 0 {
			// Cached content judgments that clear the lower bar become hits for
			// free.
	...
	fn select(&mut self, ctx: &mut Ctx) {
		// Tier 0: rank-relative to the best folder seen. While settling, only
		// names at least as promising as the best hit qualify.
		...
		if self.bar.is_none() {
			let cands: Vec<usize> = self
				.dirs
				.iter()
				.filter(|(s, i)| *s >= self.peek_floor && *s < dir_floor && !self.peeked.contains(i))
				.map(|x| x.1)
				.take(24)
				.collect();
			if !cands.is_empty() {
		...
		while !self.stop && self.reads_out < self.k && self.reads_total < read_cap {
			...
			// A clearly stronger candidate is already being read: wait for its
			// verdict before spending reads on weaker names.
	...
	fn feed(&mut self, ctx: &mut Ctx) {
		// `*_out` already count queued jobs; only respect the total in-flight
		// cap.
		let mut in_flight =
