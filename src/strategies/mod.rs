...
pub fn make(name: &str) -> Option<Box<dyn Strategy>> {
    match name {
        ...
        "inline" => Some(Box::new(inline::Inline::new(
            inline::HeatMode::PerFile,
            true,
        ))),
        "inline-shared" => Some(Box::new(inline::Inline::new(
            inline::HeatMode::Shared,
            true,
        ))),
        "inline-solo" => Some(Box::new(inline::Inline::new(
            inline::HeatMode::PerFile,
            false,
        ))),
        "inline-16k" => Some(Box::new(inline::Inline::with_cap(
            inline::HeatMode::PerFile,
            true,
            Some(16 * 1024),
        ))),
