...
//!
//! The frontier is listed and judged by name exactly like the baseline (one Noul
//! per entry). But a file whose name is hot is not given its own request: its
//! first bytes are read at once and put on a file-expand queue. Whenever a request
//! is formed, the queue is drained greedily into `state.files` (line-numbered
//! content after the tree listing) until a content budget is hit. A file that
//! does not fit is not shown at all and rides on the next request — piggybacking
//! on the next listing batch, or a files-only request when no listing is pending.
//!
//! Heat modes: one Choice per expanded file over its line ranges (`PerFile`), or
//! the user's literal suggestion of one shared Choice whose options are every
//! expanded file's ranges (`Shared`). Relevance is always a per-file Noul.

use super::Strategy;
use crate::ctx::Ctx;
use crate::jev::{NoulCriteria, Question};
use crate::pool::{Job, Outcome};
use crate::questions::{self, entry_key, line_ranges, range_key, read_text, tag_lines};
use crate::tree::{HeatRange, Kind, State};
...
// input tokens; line-prefixed code ≈ 1.8 bytes/token). Hard limits are 32k tokens
// for state + longest question and 64k per request; keep a margin.
...
fn fits(n_entries: usize, content_bytes: usize, n_files: usize) -> bool {
    ...
    let total =
        FILE_Q_TOK.mul_add(n_files as f64, ENTRY_TOTAL_TOK.mul_add(n_entries as f64, BASE_TOK) + content);
...
const TASK: &str = "Semantic grep over a source tree. `entries` lists unexplored paths to judge by name and location; `files` holds the first bytes of candidate files (every line prefixed with its number, e.g. L0042|) to judge by actual content.";

pub struct Inline {
    heat: HeatMode,
    /// Per-file content cap override (bytes); None = opts.bytes.
    cap_override: Option<usize>,
    /// Attach queued files to listing requests (true) or always send them files-only (false).
    piggyback: bool,
    frontier: Vec<usize>,
    ...
    max_depth: u32,
    file_cap: usize,
    /// How many failed requests a node has already been part of (re-queued up to `MAX_REQUEUE`).
...
impl Inline {
    ...
    /// Same eager fill as the baseline: list the shallowest unjudged folders until
    /// the frontier reaches the soft batch target.
    ...
    /// `n_entries` listing entries) stays under the token limits. A file that does
    /// not fit is left on the queue for the next request.
    ...
    fn dispatch(&mut self, ctx: &mut Ctx) {
        ...
        // Files-only requests: when the budget is full, or whenever a worker is idle
        // (everything on the queue arrived together with the last outcome, so there
        ...
        if !files.is_empty() {
            ...
            for (k, f) in files.into_iter().enumerate() {
                ...
                let note = if f.truncated {
                    format!(
                        "only the first {} of {} bytes are shown",
                        f.bytes, node.size
                    )
                } else {
                ...
                fmap.insert(
                    key.clone(),
                    json!({ "path": node.rel, "note": note, "content": tagged }),
                );
                questions.insert(
                    format!("{key}_relevant"),
                    Question::Noul {
                        instructions: Value::String(format!(
                            "Does the content of `files.{key}.content` (file \"{}\") actually contain what this search is looking for: \"{query}\"?",
                            node.rel
                        )),
                    ...
                    HeatMode::PerFile => {
                        if ranges.len() >= 2 {
                            questions.insert(
                                ...
                                Question::Choice {
                                    instructions: Value::String(format!(
                                        "Which range of lines in `files.{key}.content` (file \"{}\") best matches this search: \"{query}\"? Prefer the range where it is implemented or defined over ranges that merely import or reference it.",
                                        node.rel
                                    )),
                                    criteria,
                                },
            ...
            if self.heat == HeatMode::Shared && shared.len() >= 2 {
                questions.insert(
                    ...
                    Question::Choice {
                        instructions: Value::String(format!(
                            "Across all expanded files in `files`, which single range of lines best matches this search: \"{query}\"? Options name the file key and its line range. Prefer the range where it is implemented or defined over ranges that merely import or reference it."
                        )),
                        criteria: shared,
                    },
                );
    ...
    /// Read a hot file's head right now (page-cache fast) and queue it for inlining.
