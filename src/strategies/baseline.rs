...
//! - No hits → lower τ, reopen collapsed nodes from cached scores (no repeat
//!   asks).
...
impl Baseline {
	...
	/// Chunk the frontier into batches when it is full, or whenever there are
	/// idle workers (a small batch beats an idle worker). Then feed the pool.
	fn dispatch(&mut self, ctx: &mut Ctx) {
		...
		if !self.frontier.is_empty() && (self.frontier.len() >= ctx.opts.batch || idle) {
			...
			// Request latency grows superlinearly with entries (29 → 0.5 s, 162 →
			// 0.7 s, 255 → 1.7 s measured), so split evenly into ~N-sized
			// requests rather than packing to the cap; they run in parallel
			// anyway.
			...
			Outcome::Ask { id, result, elapsed } => {
				...
				match result {
					...
					Err(e) => {
						...
						ctx.ui
							.error(&format!("directory batch failed ({e}); re-queued {requeued} entries"));
					},
				}
			},
					...
					Err(e) => {
						if matches!(e, FileErr::Api(_)) {
							...
							*a += 1;
							if *a <= 1 {
								ctx.ui
									.error(&format!("{}: {e}; re-queued", ctx.tree.nodes[node].rel));
