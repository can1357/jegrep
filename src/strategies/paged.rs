...
//! fair-share page per open cursor (page = ⌊cap / K⌋ entries, K = open cursors),
//! one Noul per shown entry. After a page: hot files → content check, hot
//! subfolders → new cursors (K grows, pages shrink), and the cursor scrolls to its
//! next page. A cursor that reaches its end collapses with every score cached.
//!
//! Knobs:
//! - `grep`: build a keyword grep index up front (pi's grep core) and order every
//!   cursor's entries by hit count, so page 1 is the most promising slice.
//! - `strikes`: collapse a cursor early after this many consecutive cold pages
//!   (its unseen remainder is parked and paged again in the next threshold round).
//! - `labels`: also show the hit counts to the model in each entry's label.

use super::Strategy;
use crate::ctx::Ctx;
use crate::grep::{GrepIndex, grep_index, keywords_from_query};
...
impl Strategy for Paged {
    fn run(&mut self, ctx: &mut Ctx) {
        ...
        // Root: its files are one cursor; every root-level folder opens as its own cursor.
        ...
        let summary = format!(
            "paged: {} waves · {} cursors exhausted (avg {:.1} pages, max {}) · {} collapsed early · {} retries",
            self.waves,
            self.pages_done.len(),
            avg,
            max,
...
impl Paged {
    ...
    /// One page per eligible cursor, fair share of the request cap. Cursors beyond
    /// the cap round-robin into further requests (all dispatched together).
    fn dispatch_wave(&mut self, ctx: &mut Ctx) {
        ...
        for group in groups {
            ...
            if self.labels {
                // Overlay keyword counts onto the listing without touching dir_batch itself.
    ...
    /// Bookkeeping after one page of a cursor came back: scroll, collapse, or park.
    fn after_page(&mut self, ctx: &mut Ctx, dir: usize, hot: u32) {
        ...
        if let Some(limit) = self.strikes
            && cur.cold_streak >= limit && cur.hot == 0 && dir != 0 {
                let cur = self.cursors.remove(ci);
                ...
                ctx.ui.note(&format!(
                    ...
                    crate::ui::fit(&rel, 52),
                    ctx.ui.dim(&format!(
                        "{} cold pages → collapsed, {left} entries unseen",
                        cur.pages
                    ))
                ));
    ...
    /// New round, lower bar: parked cursors resume, collapsed folders whose cached
    /// score now clears the bar open, cached file scores resolve without re-asking.
            ...
            match n.kind {
                Kind::Dir => {
                    if !n.children.is_empty() {
                        // exhausted cursor: its children carry their own cached scores
...
impl Paged {
    /// Grep as a complementary signal, owned by code: when lowering the bar, also
    /// open the few still-collapsed folders / unread files with the most keyword
    /// hits, regardless of their (borderline, context-dependent) name score.
