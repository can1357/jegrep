...
use crate::{
	...
	jev::Question,
	pool::{Job, Outcome},
	questions::{self, FileErr, TASK, dir_criteria, heat_from},
	tree::{Kind, State},
};

/// Options per Choice, leaving room for `none_of_these` under the 255 cap.
const CHOICE_MAX: usize = 254;
/// Soft cap on listed entries per request (~20 tokens each → well under 32k
/// state).
...
#[derive(Default)]
pub struct Beam {
	/// Groups of units; a group (a dir + its lookahead) stays within one
	/// request.
	groups_next: Vec<Vec<Unit>>,
	ready_asks:  VecDeque<Job>,
	ready_files: VecDeque<Job>,
	asks:        HashMap<u64, Vec<Unit>>,
	/// Rescue batches (flat per-entry Nouls over the pool): request id →
	/// entries.
	...
	in_flight:   usize,
	next_id:     u64,
	/// Node → (sum of ln edge probabilities, number of decisions) along its
	/// path.
	...
	fn run(&mut self, ctx: &mut Ctx) {
		...
		'rounds: for (r, &tau) in thresholds.iter().enumerate() {
			...
			if r > 0 {
				...
				// per-entry Nouls (absolute scores), then resume the beam at this
				// bar.
				self.rescue(ctx);
				// Files must clear τ (a read costs ~10k tokens); listing a folder
				// is cheap, so folders may descend at half the bar.
		...
		if !self.closing && self.hits < ctx.opts.min_hits {
			...
			// unread file (free, local) and judge each name once, flat, at the
			// final bar.
...
impl Beam {
	...
	fn run_waves(&mut self, ctx: &mut Ctx) -> bool {
		...
		loop {
			...
			self.apply(ctx, out);
			if self.closing {
				// A listing that landed after the first hit may reveal a strong
				// file.
				self.sweep_strong(ctx);
			}
		}
	...
	/// List the whole remaining tree and queue flat name judgments for every
	/// unread file.
	fn last_resort(&mut self, ctx: &mut Ctx) {
		let mut stack: Vec<usize> = (0..ctx.tree.nodes.len())
			.filter(|&i| ctx.tree.nodes[i].kind == Kind::Dir && ctx.tree.nodes[i].state == State::Unk)
			.collect();
		...
		let mut files: Vec<usize> = (0..ctx.tree.nodes.len())
			.filter(|&i| ctx.tree.nodes[i].kind == Kind::File && ctx.tree.nodes[i].state == State::Unk)
			.collect();
	...
	fn units_for(dir: usize, kids: &[usize]) -> Vec<Unit> {
		...
		kids
			.chunks(CHOICE_MAX)
			.enumerate()
			.map(|(part, chunk)| Unit { dir, chunk: chunk.to_vec(), part, parts })
			.collect()
	}
	...
	/// Pick the next wave from the cached pools: top-K unexpanded dirs, top-M
	/// unread files.
	...
	/// Pack groups into requests (≤ `REQ_MAX_OPTIONS` entries, ≤ `REQ_MAX_UNITS`
	/// units), then feed the pool.
	...
	fn apply(&mut self, ctx: &mut Ctx, out: Outcome) {
		match out {
			Outcome::Ask { id, result, elapsed } if self.rescues.contains_key(&id) => {
				...
				match result {
					Ok(resp) => {
						...
						for (i, &idx) in entries.iter().enumerate() {
							...
							// Absolute score replaces the squashed Choice-derived one,
							// and the path restarts here so descendants are not
							// dragged down by the edge that squashed it.
					...
					Ok(resp) => {
						...
						for (u, unit) in units.iter().enumerate() {
							...
							*g = g.max(any);
							...
							let (sum_log, dec) = self.path.get(&unit.dir).copied().unwrap_or((0.0, 0));
							...
							for (j, &c) in unit.chunk.iter().enumerate() {
								...
								// Dirs: geometric mean along the path (descend
								// decision). Files: own share of this dir's
								// distribution — a strong parent edge must not
								// carry a weak leaf into a 32 KB read.
	...
	/// Flat per-entry Nouls over the pool (everything listed but never judged on
	/// its own). Gives absolute scores to entries a wide Choice squashed — the
	/// beam's known failure mode when the path to the answer runs through an
	/// innocuous name.
	...
	/// First hit landed: also read the pool files that are strong on their own,
	/// so a lucky early hit does not hide the obvious answer sitting one wave
	/// away.
	...
	/// Lower bar: cached content scores that now clear τ become hits, no
	/// re-asks.
...
/// State carries every unit's listing (`dirs.dNN.entries`); the Choice criteria
/// are null so both the `which` Choice and the `any` Noul read the same
/// listing.
fn build_request(ctx: &Ctx, units: &[Unit], query: &str) -> (Value, BTreeMap<String, Question>) {
	...
	for (u, unit) in units.iter().enumerate() {
		...
		criteria.insert(
			"none_of_these".into(),
			Value::String(
				"No listed entry plausibly contains, or leads to, a file matching the search.".into(),
			),
		);
		dirs.insert(
			uid.clone(),
		...
		questions.insert(format!("{uid}_which"), Question::Choice {
			instructions: Value::String(format!(
				"Which entry listed in `dirs.{uid}.entries` (folder \"{path}\") is the most likely \
				 place to find what this search is looking for: \"{query}\"? For a file, judge \
				 whether the file itself would contain it; for a folder (name ends with /), judge \
				 whether it contains such a file at any depth. Choose none_of_these if no listed \
				 entry plausibly does."
			)),
			criteria,
		});
		questions.insert(format!("{uid}_any"), Question::Noul {
			instructions: Value::String(format!(
				"Judging by the folder \"{path}\" and its entries listed in `dirs.{uid}.entries`, \
				 does it contain at any depth at least one file relevant to this search: \"{query}\"?"
			)),
