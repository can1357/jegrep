...
//! Every submitted passage maps back to original line numbers; absolute per-passage
//! Nouls are comparable across requests and overlapping accepted passages merge.
//! No benchmark labels, symbols, or supplemental oracle keywords are consulted.

use super::Strategy;
use crate::ctx::Ctx;
use crate::grep::{self, GrepIndex};
...
pub(super) fn passage_request(
    ...
    let criteria = NoulCriteria {
        yes: "This passage contains an implementation, definition, or substantive explanation of an important part of the search. A helper implementing one requested step counts even when other steps are elsewhere.".into(),
        no: "This passage only mentions, calls, imports, tests, or configures the subject, or contains unrelated code sharing keywords.".into(),
    };
    ...
    for (k, passage) in passages.iter().enumerate() {
        ...
        let instructions = if compact {
            ...
            entries.insert(key.clone(), json!({"start_line": passage.start, "end_line": passage.end, "content": passage.text}));
            format!(
                "Does passage `{key}` of file `{file}` substantively implement, define, or explain part of `search`? Judge only this passage, considering its original line numbers; apply `criteria`."
            )
        };
        questions.insert(
            key,
    ...
    if !compact {
        ...
        state["note"] = json!(
            "Passages retain original file line numbers. Omitted gaps are not evidence. Judge each passage independently, including separate helpers for distinct requested steps."
        );
    }
    (state, questions)
}
...
/// used for ranking the merged span without rewarding repeated/overlapping asks.
...
impl Strategy for Window {
    fn run(&mut self, ctx: &mut Ctx) {
        ...
        ctx.ui.note_entry("window", "candidate scan", None, &format!(
            "{} local files; {} query-derived candidates; up to {file_limit} content files × {per_file} windows of {window_bytes} B{}",
            index.files_scanned, ranked.len(), if adaptive { format!("; 2-window scouts below {scout_tau:.2}") } else { String::new() }));
                ...
                match result {
                    Ok(resp) => {
                        ...
                        if invalid {
                            ctx.stats.errors += 1;
                            ctx.ui.error(
                                "window candidate batch: missing or invalid filename judgment",
                            );
                        }
                    }
                    Err(error) => {
            ...
            } = outcome
            {
                ...
                match result {
                    Ok(resp) => {
                        ...
                        ctx.stats.record(resp.usage, elapsed);
                        let accumulated = results.entry(check.node).or_insert((
                            0.0,
                            Vec::new(),
                            0,
                            check.truncated,
                ...
                if *remaining == 0 {
                        ...
                        results.remove(&check.node)
                    {
                        if prior_hits.contains(&check.node) {
                            score =
                                score.max(ctx.tree.nodes[check.node].content_score.unwrap_or(0.0));
...
mod tests {
    ...
    fn compact_content_preserves_source_and_original_coordinates() {
        ...
        let (state, questions) = passage_request(
            "needle search",
            "some/file.rs",
    ...
    fn merging_retains_only_supported_lines_and_does_not_bridge_gaps() {
        ...
        assert_eq!(
            (merged[0].start, merged[0].end, merged[0].p),
            (41, 50, 0.95)
        );
    ...
    fn prior_choice_heat_survives_new_content_misses() {
        ...
        let new = vec![HeatRange {
            start: 400,
            end: 420,
            p: 0.1,
            snippet: "new miss".into(),
        }];
        let combined = combine_heat(new, &prior, 0.2);
        assert_eq!(combined.len(), 1);
        assert_eq!(
            (combined[0].start, combined[0].end, combined[0].p),
            (100, 120, 0.03)
        );
    }
    ...
    fn best_windows_include_late_content_and_empty_query_spreads_samples() {
        ...
        assert_eq!(
            selected.iter().map(|p| p.start).collect::<Vec<_>>(),
            vec![1, 41, 91]
        );
    }
}
