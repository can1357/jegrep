...
//! Instead candidates get a cheap *sniff*: the first ~1.5 KB of many files
//! packed into one request, one Noul per file ("does the opening — imports,
//! header comment, first definitions — indicate this file contains what the
//! search describes?"). Only files whose opening passes get the full read +
//! heatmap.
//!
//! Modes (env `JEGREP_SNIFF_MODE`):
//!   name  — files are name-judged first at a low bar (`name_frac` × τ), then
//! sniffed   [default]   all   — every listed file is sniffed directly; files
//! never get a name judgment
//!
//! Knobs (env): `JEGREP_SNIFF_BYTES` (1024), `JEGREP_SNIFF_LINES` (30),
//! `JEGREP_SNIFF_PER_REQ` (32),   `JEGREP_SNIFF_NAME_FRAC` (0.6),
//! `JEGREP_SNIFF_PASS_FRAC` (0.9), `JEGREP_SNIFF_FALLBACK` (3),
//!   `JEGREP_SNIFF_HEAD` (head | skeleton — skeleton = leading comment +
//! column-0 declarations   sampled from the first 32 KB; benchmarked worse than
//! the literal head on TS: it drops   the import block, which is the strongest
//! signal). Defaults are the measured best operating point (bench run 3/4).
//!
//! Fallback: if a round ends with no hits, the top-K sniff-scored files that
//! were never fully read are read anyway (openings do not always reveal
//! purpose).
...
use crate::{
	ctx::Ctx,
	pool::{Job, Outcome},
	questions::{self, FileErr, heat_from, read_text},
	tree::{Kind, State, human_size},
};
...
enum HeadKind {
	...
	Head,
	/// Leading comment + column-0 declarations sampled from the first 32 KB
	/// (`JEGREP_SNIFF_HEAD=skeleton`).
	Skeleton,
}
...
pub struct Sniff {
	k:             Knobs,
	frontier:      Vec<usize>,
	/// Ask jobs (directory batches and sniff batches), dispatched before file
	/// jobs.
	ready:         VecDeque<Job>,
...
impl Sniff {
	...
	/// Same eager fill as the baseline: list shallowest unjudged folders until
	...
	fn dispatch(&mut self, ctx: &mut Ctx) {
		...
		// Heads → sniff batches. Full batches always; partial ones when workers
		// idle.
	...
	/// Read a file's head on the main thread (tiny local I/O) and stage it for a
	/// sniff batch.
	fn sniff_read(&mut self, ctx: &mut Ctx, idx: usize) {
		let path = ctx.tree.nodes[idx].path.clone();
		// Skeleton scans the same span a full read would (local I/O only) but
		// sends ≤ sniff_bytes of it; the bytes counted are the bytes sent.
		...
		match read_text(&path, scan) {
			Ok(rt) => {
				let head = match self.k.head {
					...
					HeadKind::Skeleton => {
						questions::skeleton_excerpt(&rt.text, self.k.sniff_lines, self.k.sniff_bytes)
					},
				};
				let sent = head.len();
				ctx.stats.sniffed += 1;
	...
	/// No hits this round: fully read the best-sniffed files that were never
	/// read.
