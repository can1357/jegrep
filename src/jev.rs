...
//! typed questions (noul = yes/no probability, choice = distribution over options).
...
fn providers(
    ...
    if providers.is_empty() {
        return Err(
            "set OPENROUTER_API_KEY or TYPESAFE_API_KEY in the environment or ~/.env".into(),
        );
    }
    Ok(providers)
}
...
/// Total retries and failover attempts performed by every client in this process.
...
pub struct Client {
    ...
    model: String,
    max_retries: u32,
    /// Opt-in limit for independent Noul questions per HTTP request. Each logical
    /// call has at most two chunks in flight, so strategy --parallel P permits
    /// up to 2*P physical requests. Choice and mixed batches remain intact.
            ...
            match sent {
                Ok(mut resp) => {
                    ...
                    if retry && transient && attempt < self.max_retries {
                        ...
                        if !WARNED.swap(true, std::sync::atomic::Ordering::Relaxed) {
                            crate::ui::diagnostic(&format!(
                                "  ! jev: HTTP {status}, backing off (further retries are counted silently; see footer)"
                            ));
                        }
                ...
                Err(e) => {
                    let transient = !matches!(
                        e,
                        ureq::Error::BadUri(_) | ureq::Error::Http(_) | ureq::Error::Tls(_)
                    );
                    if retry && transient && attempt < self.max_retries {
                        attempt += 1;
                        RETRIES.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
...
mod tests {
    ...
    fn provider_selection() {
        ...
        assert_eq!(
            providers(None, only_typesafe).unwrap()[0].url,
        ...
        let handle = thread::spawn(move || {
            ...
            while workers.len() < count {
                ...
                workers.push(thread::spawn(move || {
                    ...
                    let answers: BTreeMap<_, _> = request["questions"].as_object().unwrap().keys()
                        .map(|key| (key, serde_json::json!({"type": "noul", "noul": 0.75}))).collect();
                    let n = answers.len();
                    let body = serde_json::json!({"model": "jev-latest", "answers": answers,
                        "usage": {"input_tokens": 100 + n * 10, "output_tokens": n}}).to_string();
                    write!(stream, "HTTP/1.1 {status} Test\r\nContent-Length: {}\r\nContent-Type: application/json\r\nConnection: close\r\n\r\n{body}", body.len()).unwrap();
