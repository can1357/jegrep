...
pub struct Case {
	pub name:   String,
	pub query:  String,
	/// Acceptable answers: exact root-relative file paths, or a folder prefix
	/// ending in `/`.
...
pub fn run(
	...
	for case in cases {
		let opts = Opts { query: case.query.clone(), ..base.clone() };
		let ui = Ui::new(UiOptions { quiet: !verbose, verbose, ..UiOptions::default() });
...
pub fn summarize(rows: &[Row]) {
	...
	// latest row wins when a (strategy, case) pair was benchmarked more than
	// once
