...
//!    ("is this path likely to hold what the search wants?"). Nouls give absolute
//!    probabilities, so entries can be thresholded independently and batches are
...
use crate::grouped::{self, Leaf};
use crate::jev::{NoulCriteria, Question, Response};
use crate::tree::{HeatRange, Kind, Tree, human_size};
...
pub const TASK: &str = "Semantic grep over a source tree: locate files whose content matches the search description. Entries are judged by name, size, position in the tree, and (for folders) a sample of what they contain.";

pub const TREE_FORMAT: &str = "`tree` is a directory listing. Lines starting with # are headers: `# dir/` is a folder; more #s means deeper nesting under the header above; a header may fold several levels (`# a/b/c/`). Every judgeable entry carries a tag like e017 right after the #s: files as `e017 name (size)`, folders as `e017 name/ — N entries: sample of names`. Untagged header lines are only structure.";
...
pub fn file_criteria() -> NoulCriteria {
    NoulCriteria {
        ...
        no: "Unrelated by name and location, or a kind of file (generated output, lockfile, asset, boilerplate) that would not hold it.".into(),
    }
}

pub fn dir_criteria() -> NoulCriteria {
    NoulCriteria {
        yes: "The folder plausibly contains, at any depth, at least one file matching the search.".into(),
        no: "Nothing about the folder's name, location, or sampled contents suggests it holds a match.".into(),
    }
}

/// Detail shown after an entry's name: size for files, count + sample for folders.
...
/// Render the frontier entries as pi's model-facing grouped tree, each judgeable
/// entry tagged `eNNN` (the question key). Returns the text and, per entry, its tag.
...
/// How per-entry questions are phrased. Selected with `JEGREP_QFMT` for benchmarking:
/// `full` repeats the yes/no criteria in every question and inlines the query;
/// `lean` states the criteria once in the state and inlines the query;
/// `leanref` states the criteria once and refers to `search` in the state.
...
pub fn dir_batch_fmt(
    ...
    for (i, &idx) in entries.iter().enumerate() {
        ...
        let (instructions, criteria) = match (fmt, n.kind) {
            (QFmt::Full, Kind::File) => (
                format!(
                    "Judging by its name, size, and where it sits in `tree`, is the file tagged {key} (\"{}\") likely to contain what this search is looking for: \"{query}\"?",
                    n.name()
                ),
                Some(fc.clone()),
            ),
            (QFmt::Full, Kind::Dir) => (
                format!(
                    "Judging by its name, its place in `tree`, and the sample of its contents, does the folder tagged {key} (\"{}/\") contain, at any depth, at least one file relevant to this search: \"{query}\"?",
                    n.name()
                ),
                Some(dc.clone()),
            ),
            (QFmt::Lean, Kind::File) => (
                format!(
                    "Is the file tagged {key} (\"{}\") likely to contain what this search is looking for: \"{query}\"? Judge by its name, size, and place in `tree`; apply `criteria.file`.",
                    n.name()
                ),
                None,
            ),
            (QFmt::Lean, Kind::Dir) => (
                format!(
                    "Does the folder tagged {key} (\"{}/\") contain, at any depth, at least one file relevant to this search: \"{query}\"? Judge by its name, place in `tree`, and sampled contents; apply `criteria.folder`.",
                    n.name()
                ),
                None,
            ),
            (QFmt::LeanRef, Kind::File) => (
                format!(
                    "Is the file tagged {key} (\"{}\") likely to contain what `search` describes? Judge by its name, size, and place in `tree`; apply `criteria.file`.",
                    n.name()
                ),
                None,
            ),
            (QFmt::LeanRef, Kind::Dir) => (
                format!(
                    "Does the folder tagged {key} (\"{}/\") contain, at any depth, at least one file relevant to `search`? Judge by its name, place in `tree`, and sampled contents; apply `criteria.folder`.",
                    n.name()
                ),
                None,
            ),
        ...
        questions.insert(
            ...
            Question::Noul {
                instructions: Value::String(format!(
                    "Given the full list of entries in folder `folders.{key}` (\"{}\"), does it contain, at any depth, at least one file relevant to this search: \"{query}\"?",
                    n.rel
                )),
...
/// Read up to `max_bytes` of a text file. Rejects binaries (NUL in the first 8 KB)
...
/// `(start, end, snippet)` per range (1-based inclusive) and the matching Choice criteria.
pub fn line_ranges(
    lines: &[&str],
    n_ranges: usize,
    ...
    while start < lines.len() {
        ...
        let snippet: String = lines[start..end]
            ...
            .collect();
        criteria.insert(
            range_key(ranges.len()),
            Value::String(format!("lines {}-{}", start + 1, end)),
        );
...
pub fn relevant_noul(rel: &str, query: &str) -> Question {
    Question::Noul {
        instructions: Value::String(format!(
            "Does the content of \"{rel}\" (in `content`) actually contain what this search is looking for: \"{query}\"?"
        )),
        criteria: Some(NoulCriteria {
            yes: "The file contains code, text, or data that directly matches, implements, defines, or documents what the search describes.".into(),
            no: "The file is unrelated, or only shares surface keywords with the search without containing the thing itself.".into(),
        }),
    }
}
...
pub fn where_choice(query: &str, criteria: BTreeMap<String, Value>) -> Question {
    Question::Choice {
        instructions: Value::String(format!(
            "Which range of lines in `content` (every line is prefixed with its number, e.g. L0042|) best matches this search: \"{query}\"? Prefer the range where it is implemented or defined over ranges that merely import or reference it."
        )),
        criteria,
    }
}
...
pub struct FilePrep {
    ...
    /// (`start_line`, `end_line`, snippet) per range, in document order; keys are R00..
...
/// The standard content check: first `max_bytes`, relevance Noul + heatmap Choice.
pub fn prepare_file(
    path: &Path,
    rel: &str,
    size: u64,
...
/// First `max_lines` lines of a file head, long lines clipped, for a sniff batch.
...
/// then column-0 declarations (exports, functions, classes, types, headings…) in
/// file order, with a few imports to fill, capped at `max_lines` / `max_bytes`.
/// Same token budget as a head excerpt, but samples the whole scanned text.
...
pub fn sniff_batch(
    ...
    let criteria = NoulCriteria {
        yes: "The imports, header comment, or first definitions show the file implements, defines, or documents what the search describes, or strongly indicate the rest of the file does.".into(),
        no: "The opening shows an unrelated purpose, or shares only surface keywords with the search.".into(),
    };
    ...
    for (i, (rel, size, head)) in files.iter().enumerate() {
        ...
        listing.insert(
            key.clone(),
            json!({ "path": rel, "size": size, "head": head }),
        );
        questions.insert(
            key.clone(),
            Question::Noul {
                instructions: Value::String(format!(
                    "Judging from its path and opening lines (`files.{key}.head`), does the file \"{rel}\" likely contain what this search is looking for: \"{query}\"?"
                )),
