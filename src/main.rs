...
The default cascade strategy ranks candidates using query-derived keywords, judges filenames, \
then uses small source sketches to select up to 40 full passages for parallel verification. \
Only verified source supplies results, with original line numbers and merged adjacent passages. \
Use --list-strategies to explore other policies."
)]
struct Cli {
    ...
    min_hits: usize,
    /// Preferred API provider (default: `OpenRouter` when its key exists). Fails over if both keys exist.
    #[arg(long, value_enum)]
    endpoint: Option<jev::Endpoint>,
    /// Jev model id or alias.
    #[arg(long, default_value = "jev-latest")]
    model: String,
    /// Extra grep keywords for grep-prior strategies (comma-separated; default: derived from the query).
    ...
    #[arg(long)]
    json: bool,
    /// Run a benchmark: a JSON file of {name, query, expect[]} cases over PATH (the lone positional). Prints a table.
    ...
    /// With --bench: label rows with this name instead of the strategy name (to compare variants).
    #[arg(long, value_name = "LABEL")]
    bench_label: Option<String>,
    /// Summarize one or more --bench-out JSONL files into a per-strategy comparison and exit.
...
fn main() {
    ...
    // `jegrep --bench cases.json PATH`: the lone positional is the path, not a query.
    if cli.bench.is_some() && cli.path == *"."
