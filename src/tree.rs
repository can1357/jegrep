//! The explored directory tree. Nodes are created lazily as folders are expanded.
//!
//! State machine per node:
//!   Unk ──(dir batch)──▶ Pending ──▶ Exp (dir, hot)  | Fin(round) (cold)
//!                                 ──▶ Reading (file, hot) ──▶ Hit | Fin(round)
//!   Fin(r) can reopen in a later round when the cached score clears the new threshold.
...
impl Tree {
    ...
    /// Returns the new child indices. Marks the node `Exp` (or `Skip` when empty).
    ...
    /// without creating child nodes. Used for cheap "full listing" judgments.
    ...
    /// Count eligible entries and sample a few names. `None` when nothing eligible.
