...
use crate::walker::{CollectedEntries, CollectedEntry, FileType, WalkError, WalkOptions};
...
#[cfg(test)]
pub fn cache_test_guard() -> parking_lot::MutexGuard<'static, ()> {
...
mod tests {
	...
	use crate::walker::{CollectedEntry, FileType};
	...
	fn key(name: &str) -> super::CacheKey {
		super::cache_key(Path::new(name), crate::walker::WalkOptions::default())
	}
	...
	fn collect_entries_discards_a_scan_invalidated_while_in_flight() {
		...
		let options = crate::walker::WalkOptions {
			cache: true,
			..scan_options(true, false, crate::walker::WalkDetail::Minimal)
		};
	...
	fn cache_hits_preserve_owned_results_and_respect_cancellation() {
		...
		assert!(
			matches!(cancelled, Err(crate::walker::WalkError::Interrupted(error)) if error == "cancelled")
		);
		...
		assert!(
			matches!(cancelled_after_copy, Err(crate::walker::WalkError::Interrupted(error)) if error == "cancelled after copy")
		);
		super::invalidate_path(root.path());
	}
	...
	fn scan_options(
		include_hidden: bool,
		use_gitignore: bool,
		detail: crate::walker::WalkDetail,
	) -> crate::walker::WalkOptions {
		crate::walker::WalkOptions {
			include_hidden,
			use_gitignore,
			skip_git: true,
			skip_node_modules: true,
			follow_links: crate::walker::FollowLinks::Never,
			detail,
			directory_errors: crate::walker::DirectoryErrorMode::SkipSkippable,
			..crate::walker::WalkOptions::default()
		}
	}
	...
	fn collect_entries_skips_node_modules() {
		...
		let entries = super::collect_entries(
			root.path(),
			scan_options(true, false, crate::walker::WalkDetail::Full),
			ok_heartbeat,
		)
		.unwrap();
	...
	fn collect_entries_follow_links_always() {
		...
		let mut options = scan_options(true, false, crate::walker::WalkDetail::Minimal);
		options.follow_links = crate::walker::FollowLinks::Always;
	...
	fn traversal_gitignore_excludes_files() {
		...
		let collected = super::collect_entries(
			root.path(),
			scan_options(true, true, crate::walker::WalkDetail::Full),
			ok_heartbeat,
		)
		.unwrap();
	...
	fn traversal_hidden_disabled_excludes_files_and_descendants() {
		...
		let entries = super::collect_entries(
			root.path(),
			scan_options(false, false, crate::walker::WalkDetail::Full),
			ok_heartbeat,
		)
		.unwrap();
		let entries = entries.entries;
