//! Final report for a single search: pretty text (default), JSON, optional tree.
...
pub fn print(ctx: &Ctx, json: bool, tree: bool) {
    ...
    if hits.is_empty() {
        ...
        if !misses.is_empty() {
            ...
            let items: Vec<(String, f64)> = misses
                ...
                .map(|&i| {
                    (
                        ctx.tree.nodes[i].rel.clone(),
                        ctx.tree.nodes[i].content_score.unwrap_or(0.0),
                    )
                })
                .collect();
        ...
        if !names.is_empty() {
            ...
            let items: Vec<(String, f64)> = names
                ...
                .map(|&i| {
                    (
                        ctx.tree.nodes[i].rel.clone(),
                        ctx.tree.nodes[i].name_score.unwrap_or(0.0),
                    )
                })
                .collect();
            ui.print_scored(&mut out, &items);
    ...
    let _ = writeln!(
        ...
        "{}",
        ui.dim(&format!(
            "listed {listed} · judged {} · expanded {} dirs · read {} files ({}){} · {} requests{}{} · {} tokens · ${:.4} · {:.1}s wall / {:.1}s api",
            s.judged,
            s.expanded,
            s.files_read,
            ...
            s.requests,
            if s.errors > 0 { format!(" ({} failed)", s.errors) } else { String::new() },
            {
                let r = crate::jev::RETRIES.load(std::sync::atomic::Ordering::Relaxed);
                if r > 0 { format!(" ({r} retries)") } else { String::new() }
            },
